...
class CanvasFirebaseOperationsStorage extends
  ...
  _handleNewOperation(painting: CanvasPainting, operation: CanvasOperation):
    ...
    console.log('queue length before:', this._pendingOperations.length);
    ...
    console.log('found operation', pendingOperationIndex != this._pendingOperations.length);
    if (pendingOperationIndex == 0) {
      // if this is our own operation, nothing else to do since we already
      // played it.
      console.log('this is our own operation');
      this._pendingOperations = [];
      return;
    }
