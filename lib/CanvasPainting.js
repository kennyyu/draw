...
class CanvasPainting {
  ...
  getCanvasURL(): string {
    //TODO: reenable whne this is used
  ...
  clearCanvas(notifyObservers: bool = true): void {
  ...
  _startCanvasOperation(event: CanvasEvent): void {
    ...
    this._baseCanvas = this.getCanvasURL();
    this.startPath(loc);
  ...
  _nextLocCanvasOperation(event: CanvasEvent): void {
  ...
  mousemove(event: CanvasEvent): void {
    if (this._isDrawing) {
      this._nextLocCanvasOperation(event);
